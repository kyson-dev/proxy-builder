...
while (($#)); do
  case "$1" in
    ...
    *) printf '%s\n' 'usage: e2e-proxy.sh --subscription <file> --image <digest> --probe-url <https-url> --proxyctl <binary>' >&2; exit 2 ;;
  esac
done
...
for item in 'vless:18080' 'hysteria2:18081'; do
  protocol="${item%%:*}"
  port="${item##*:}"
...
printf '%s\n' 'VLESS and Hysteria2 egress probes passed'
